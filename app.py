...
def generate_docx(df_schedule, trip_name, selected_country, trip_purpose, school_details):
    ...
    # 2. 출장 개요 (사용자가 입력/수정한 목적 반영)
    doc.add_heading('Ⅰ. 출장 개요', level=1)
    doc.add_paragraph(f"1. 출장 목적 및 배경").bold = True
    ...
    for _, row in df_schedule.iterrows():
        ...
        if date_val == 'nan' or not date_val.strip(): continue
...
# --- 2. Streamlit UI ---
st.set_page_config(page_title="Yuju's Automation v5", layout="wide")
st.title("🚀 스마트 출장보고서 생성기 (계획서 연동형)")
...
with st.sidebar:
    ...
    excel_file = st.file_uploader("📅 출장 일정표(Excel) 업로드", type=["xlsx"])
    info_file = st.file_uploader("📄 운영 계획서 등 참고자료 업로드", type=["hwp", "hwpx", "pdf", "docx"])

if excel_file:
    try:
        df = pd.read_excel(excel_file, sheet_name="1. 상세 일정표", skiprows=3)
        df.columns = [c.strip() for c in df.columns]
        st.success("일정표를 성공적으로 읽었습니다!")

        # [섹션 1] 출장 목적 편집
        st.header("2. 출장 목적 및 배경 작성")
        default_purpose = f"◦ 'Study Korea 300K Project' 연계 및 {selected_country} 내 우수 인재 유치 기반 마련\n◦ 현지 교육 기관과의 네트워킹 강화 및 본교 인지도 제고"
        
        # 참고자료가 업로드된 경우 알림 (나중에 여기에 OCR/텍스트 추출 로직 연동 가능)
        if info_file:
            st.info(f"💡 업로드하신 '{info_file.name}' 자료를 참고하여 아래 내용을 다듬어 주세요.")
        
        trip_purpose = st.text_area("출장 목적 (보고서 첫 페이지에 들어갈 내용)", value=default_purpose, height=150)

        # [섹션 2] 학교 정보 편집
        st.header("3. 방문 기관별 상세 활동 내용")
        if '도착지' in df.columns:
            potential_schools = df[df['도착지'].str.contains('학교|School|Univ|기관|인텍|INTEC', na=False, case=False